  ...
  describe "Callbacks" do
    describe "#expiry_cache" do
      it "should clear the cache on save" do
        ...
        cache.getset("post_action_types_#{I18n.locale}") { "test" }
        cache.getset("post_action_flag_types_#{I18n.locale}") { "test2" }

        PostActionType.new(name_key: "some_key").save!

        expect(cache.getset("post_action_types_#{I18n.locale}") { nil }).to eq(nil)
        expect(cache.getset("post_action_flag_types_#{I18n.locale}") { nil }).to eq(nil)
      ensure
